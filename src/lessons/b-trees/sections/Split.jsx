    ...
    <Section roman="V" kicker="The split" title="It grows at the root, never the leaves">
      <Reveal base="bt-rev">
        <p className="bt-p">
          ...
          If it still fits, you&rsquo;re done &mdash; no rebalancing, nothing. If the leaf
          overflows, it <strong>splits</strong>: the median key rises into the parent, and the rest
          break into two half-full nodes. That promotion is itself an insertion into the parent, so
          a split can <span className="bt-em">cascade</span> upward.
        </p>
      </Reveal>
      <Reveal base="bt-rev">
        <p className="bt-lead">
          ...
          at the root &mdash; and that one event lifts every leaf down by a level, all at once.
        </p>
      </Reveal>
      <Reveal base="bt-rev">
        <p className="bt-p">
          ...
          The building only ever gets taller by adding a fresh lobby &mdash; so every tenant stays
          exactly the same distance from the door. That equidistance isn&rsquo;t a step you perform.
      ...
      <Callout title="Deletion is the same machine, run backward">
        ...
        key from a fat sibling, or <span className="bt-em">merges</span> with a lean one &mdash;
        pulling a key down from the parent. That can cascade and, at the very top, collapse the
        root, making the whole tree one level shorter.
      </Callout>
    </Section>
  );
}
