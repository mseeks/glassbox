...
export function Ch07bBlocked() {
  return (
    <Chapter num="07b" title="Blocked Bloom Filter" anchor="ch-07b">
      ...
      <p>
        The result: <strong>one cache miss per query</strong>, not <code>k</code>. The FPR for the
        same memory is slightly worse, because each block is now its own tiny filter with its own
        load-factor variance. But throughput in practice is multiples higher. We no longer scatter
        memory accesses.
      </p>
    </Chapter>
  );
