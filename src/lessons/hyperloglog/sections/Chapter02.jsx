  ...
  return (
    <Chapter n="02" title="The rarest flicker">
      ...
      <p>
        ...
        about eight people before witnessing even one. A run of ten, one in a thousand, would
        whisper about a thousand people.{' '}
      ...
      <div className="pull">
        The rarest run encodes the size of the crowd, so the maximum run of leading zeros becomes an{' '}
        <em>estimate of the count</em>.
      </div>
      <p>
        Below, each item is a coin sequence; the leading zeros before the first 1 are its run. Push
        the crowd size up and watch the longest run climb with it. Then run the trials repeatedly to
        feel the catch.
      </p>
      <CoinLab />
      <p>
        The idea is exactly right. The precision is hopeless. A lone coin-run can only ever guess a
        power of two, and re-rolling the very same crowd flings the answer wildly from one trial to
        the next. Everything from here fights that variance.
      </p>
    </Chapter>
  );
}
